...
function App() {
  ...
  useEffect(() => {
    // FIX: Hapus 'disable: mobile' agar elemen tidak hilang di HP
    // Animasi akan tetap jalan di HP, memastikan elemen muncul (opacity: 1)
    AOS.init({
      once: true,
      duration: 1000,
    });
  ...
  return (
    <>
      ...
      <main className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 overflow-x-hidden">
        {/* === HERO SECTION === */}
        {/* Layout: Mobile = Stack (Vertikal), Desktop = Grid 2 Kolom Rata */}
        <div className="min-h-[auto] pt-32 pb-16 md:min-h-screen md:grid md:grid-cols-2 md:items-center md:gap-16 md:pt-0 md:pb-0">
          {/* TEXT (Kiri) */}
          <div className="order-1 md:order-1 flex flex-col items-center md:items-start text-center md:text-left animate__animated animate__fadeInUp">
            ...
            <div className="mb-8 w-full max-w-lg md:max-w-none">
              <BlurText text="Crafting Digital Experiences with Code & Passion." className="text-indigo-200 text-lg sm:text-xl md:text-2xl font-light block" />
            </div>

            <div className="flex flex-wrap gap-4 justify-center md:justify-start">
              <a href="#project" className="bg-indigo-600 text-white px-8 py-3 rounded-full font-bold hover:bg-indigo-500 transition-all transform hover:scale-105 shadow-[0_0_15px_rgba(99,102,241,0.5)]">
                View My Work
              </a>
              <a href="/CV.pdf" download="CV_Ryonandha.pdf" className="border border-indigo-500/30 bg-indigo-900/20 backdrop-blur-md text-white px-8 py-3 rounded-full font-bold hover:bg-indigo-500/20 transition-all">
                Download CV
              </a>
            </div>
          </div>
          ...
          {/* PROFILE CARD (Kanan) */}
          {/* FIX: 'w-full' dan 'md:justify-end' memaksa kartu ke pojok kanan di desktop */}
          <div className="order-2 md:order-2 flex w-full justify-center md:justify-end animate__animated animate__fadeInUp animate__delay-1s relative mt-16 md:mt-0">
            <div className="absolute inset-0 bg-indigo-500 blur-[80px] opacity-20 -z-10 rounded-full transform scale-75"></div>

            {/* Wrapper agar di Mobile tidak terlalu lebar, tapi di Desktop ikut ukuran aslinya */}
        ...
        {/* === ABOUT ME SECTION === */}
        <div className="mt-10 md:mt-0 mx-auto w-full rounded-3xl border border-white/5 bg-white/5 backdrop-blur-xl p-8 md:p-12 shadow-2xl mb-20" id="about">
          <div className="flex flex-col md:flex-row items-center justify-between gap-12">
            ...
            <div className="w-full md:w-7/12 order-2 md:order-1" data-aos="fade-right">
              ...
              <p className="text-gray-300 leading-relaxed mb-8 text-base md:text-lg font-light text-justify md:text-left">
                ...
                Node.js, and AI integration, I transform complex ideas into intuitive, scalable applications. Backed by over 3 years of experience and 20+ successful projects.
              </p>
            ...
            {/* Lanyard 3D */}
            {/* FIX: Tinggi disesuaikan agar tidak error di mobile */}
            <div className="w-full md:w-5/12 h-[300px] md:h-[400px] relative order-1 md:order-2 cursor-grab active:cursor-grabbing">
        ...
        {/* === TOOLS SECTION === */}
        <div className="mb-32" id="tools">
          <h1 className="text-3xl md:text-5xl font-bold text-white mb-12 text-center" data-aos="fade-up">
            Tech <span className="text-indigo-400">Stack</span>
          </h1>
          {/* Grid Responsif: 2 kolom (HP), 3 (Tablet), 5 (Desktop) */}
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {listTools.map((tool) => (
              ...
              >
                <img src={tool.gambar} alt={tool.nama} className="w-12 h-12 md:w-16 md:h-16 object-contain mb-4 group-hover:scale-110 transition-transform filter drop-shadow-lg" />
        ...
        {/* === CONTACT SECTION === */}
        <div className="mb-32 text-center px-4" id="contact">
          ...
          <h1 className="text-4xl md:text-7xl font-bold text-white mb-10 leading-tight">
